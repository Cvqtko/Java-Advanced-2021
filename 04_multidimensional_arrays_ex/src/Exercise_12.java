...
public class Exercise_12 {
	public static void main(String[] args) throws IOException {
		...
		int rows = Arrays.stream(reader.readLine().trim().split("\\s+")).mapToInt(Integer::parseInt).findFirst()
				.orElse(0);

		char[][] matrix = reader.lines().limit(rows).map(line -> line.replaceAll("\\s+", "").toCharArray())
				.toArray(char[][]::new);

		char fillChar = reader.readLine().trim().charAt(0);

		int[] startPosition = Arrays.stream(reader.readLine().trim().split("\\s+")).mapToInt(Integer::parseInt)
				.toArray();
